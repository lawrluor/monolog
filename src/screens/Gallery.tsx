...
const Gallery = ({ navigation }: Props) => {
  ...
  const { videosData, isLoading, toggleVideosRefresh, submitQuery } = videosContext;
  ...
  const renderModal = () => {
    return (
      modalShown
        ?
        <SearchEntry editable onFinish={makeSearchQuery} modalShown={modalShown} setModal={setModal}>
